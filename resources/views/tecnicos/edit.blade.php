<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold text-gray-800">Editar Técnico</h2>
    </x-slot>

    <div class="py-6 max-w-xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="app-surface p-6">

            @if($errors->any())
                <div class="mb-4 p-4 bg-red-100 text-red-800 rounded-lg text-sm">
                    <ul class="list-disc list-inside space-y-1">
                        @foreach($errors->all() as $error)
                            <li>{{ $error }}</li>
                        @endforeach
                    </ul>
                </div>
            @endif

            <form method="POST" action="{{ route('tecnicos.update', $tecnico) }}" class="space-y-5">
                @csrf
                @method('PUT')

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Nome *</label>
                    <input type="text" name="nome" value="{{ old('nome', $tecnico->nome) }}"
                           class="app-field w-full"
                           required>
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Telefone WhatsApp *</label>
                    <input type="text" name="telefone" value="{{ old('telefone', $tecnico->telefone) }}"
                           placeholder="Ex: 5573999990000"
                           class="app-field w-full"
                           required>
                    <p class="text-xs text-gray-400 mt-1">Formato: cÃ³digo do paÃ­s + DDD + nÃºmero (sem espaÃ§os ou traÃ§os)</p>
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Grupo de envio *</label>
                    <x-sgp-select
                            name="whatsapp_grupo_id"
                            :options="$whatsappGrupos->mapWithKeys(fn ($grupo) => [$grupo->id => $grupo->nome])->all()"
                            :selected="$tecnico->whatsapp_grupo_id"
                            placeholder="Selecione um grupo"
                            class="w-full"
                        />
                    <p class="text-xs text-gray-400 mt-1">As ordens deste Técnico serÃ£o enviadas para esse grupo.</p>
                </div>

                <div class="flex items-center gap-2">
                    <input type="checkbox" name="ativo" id="ativo" value="1"
                           {{ old('ativo', $tecnico->ativo) ? 'checked' : '' }}
                           class="rounded border-slate-300 text-[#ff7a00] focus:ring-[#ff7a00]">
                    <label for="ativo" class="text-sm font-medium text-slate-700 dark:text-slate-300">Técnico ativo</label>
                </div>

                <div class="flex gap-3 pt-2">
                    <button type="submit"
                            class="app-btn-primary">
                        Atualizar
                    </button>
                    <a href="{{ route('tecnicos.index') }}"
                       class="app-btn-secondary">
                        Cancelar
                    </a>
                </div>
            </form>
        </div>
    </div>
</x-app-layout>

